...
export const trackParcel = async (trackingNumber, carrier = 'korea-post') => {
  try {
    const response = await apiService.get('/admin/parcel/track', { 
      params: { 
        tracking_number: trackingNumber, 
        carrier 
      } 
    ...
    // 백엔드 응답 구조에 따라 data.data 또는 data 자체를 반환하도록 조정
    return { success: response.status === 200, data: response.data.data, message: '택배 추적 성공' };
  } catch (error) {
    console.error('API Error: trackParcel', error);
    return { success: false, message: error.response?.data?.message || '택배 추적 중 오류가 발생했습니다.' };
  }
};
